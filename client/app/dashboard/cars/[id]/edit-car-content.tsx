'use client';

import Link from 'next/link';
import { Lock } from 'lucide-react';
import { CarForm } from '../../../../components/cars/car-form';
import type { Car } from '../../../../types/car.types';
import { useI18n } from '@/lib/i18n';

interface EditCarContentProps {
  car: Car;
}

export function EditCarContent({ car }: EditCarContentProps) {
  const { t } = useI18n();

  if (car.source === 'dubicars') {
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-2 text-sm text-gray-400">
          <Link href="/dashboard" className="hover:text-gray-700">{t('dashboardCars.breadcrumb.dashboard')}</Link>
          <span>/</span>
          <Link href="/dashboard/cars" className="hover:text-gray-700">{t('dashboardCars.breadcrumb.cars')}</Link>
          <span>/</span>
          <span className="text-gray-700">{t('dashboardCars.editTitle')}</span>
        </div>

        <div className="bg-orange-50 border border-orange-200 rounded-2xl p-8 text-center space-y-4">
          <Lock className="mx-auto h-10 w-10 text-orange-400" aria-hidden="true" />
          <h2 className="text-xl font-bold text-gray-800">{t('dashboardCars.dubicarsNoticeTitle')}</h2>
          <p className="text-gray-500 max-w-sm mx-auto text-sm leading-relaxed">
            {t('dashboardCars.dubicarsNoticeMessage')}
          </p>
          <div className="flex items-center justify-center gap-3 pt-2">
            <Link href="/dashboard/cars">
              <button className="px-5 py-2 text-sm border border-gray-200 rounded-xl hover:border-gray-400 transition-colors">
                {t('dashboardCars.backToList')}
              </button>
            </Link>
          </div>
        </div>
      </div>
    );
  }

  return (
    <div className="space-y-6">
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-center gap-2 text-sm text-gray-400 mb-2">
            <Link href="/dashboard" className="hover:text-gray-700">{t('dashboardCars.breadcrumb.dashboard')}</Link>
            <span>/</span>
            <Link href="/dashboard/cars" className="hover:text-gray-700">{t('dashboardCars.breadcrumb.cars')}</Link>
            <span>/</span>
            <span className="text-gray-700">{t('dashboardCars.editTitle')}</span>
          </div>
          <h1 className="text-2xl font-bold text-gray-900">{t('dashboardCars.editTitle')}</h1>
          <p className="text-sm text-gray-400 mt-1">
            {car.make} {car.model} {car.year}
            {car.trim && ` - ${car.trim}`}
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/cars/${car.id}`} target="_blank">
            <button className="text-sm text-gray-400 hover:text-gray-700 border border-gray-200 px-4 py-2 rounded-xl transition-colors">
              {t('dashboardCars.viewOnSite')}
            </button>
          </Link>
          <Link href="/dashboard/cars">
            <button className="text-sm text-gray-400 hover:text-gray-700 border border-gray-200 px-4 py-2 rounded-xl transition-colors">
              {t('dashboardCars.backToList')}
            </button>
          </Link>
        </div>
      </div>

      <div className={`rounded-xl px-4 py-3 text-sm flex items-center gap-2 ${
        car.status === 'active' ? 'bg-green-50 text-green-700 border border-green-100' :
        car.status === 'paused' ? 'bg-yellow-50 text-yellow-700 border border-yellow-100' :
        car.status === 'sold' ? 'bg-blue-50 text-blue-700 border border-blue-100' :
        'bg-red-50 text-red-700 border border-red-100'
      }`}>
        <span>
          {car.status === 'active' ? t('dashboardCars.statusActiveVisible') :
           car.status === 'paused' ? t('dashboardCars.statusPausedHidden') :
           car.status === 'sold' ? t('dashboardCars.statusSold') : t('dashboardCars.statusDeleted')}
        </span>
      </div>

      <CarForm car={car} />
    </div>
  );
}
